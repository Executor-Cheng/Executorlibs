...
namespace Executorlibs.TarsProtocol.IO
{
    ...
    public ref struct TarsWriter
    {
        ...
        public TarsWriter(Stream? stream) : this(stream, BufferSize)
        {

        }

        public TarsWriter(Stream? stream, int bufferSize) : this(GC.AllocateUninitializedArray<byte>(bufferSize), 0, bufferSize, stream)
        {

        }

        public TarsWriter(byte[] buffer) : this(buffer, 0, buffer.Length)
        {

        }

        public TarsWriter(byte[] buffer, int offset, int length) : this(buffer, offset, length, null)
        {

        }

        private TarsWriter(byte[] buffer, int offset, int length, Stream? underlyingStream)
        {
        ...
        public void WriteHeader(TarsHeader header)
        {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private void WriteHeaderSlowPath(TarsHeader header)
        {
            WriteRawByte((byte)header._rawTag);
            if (header.HasTag8)
            {
