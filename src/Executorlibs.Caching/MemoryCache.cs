...
namespace Microsoft.Extensions.Caching.Memory
{
    ...
    public class MemoryCache<TKey, TValue> : IMemoryCache<TKey, TValue> where TKey : notnull
    {
        ...
        private readonly MemoryCacheOptions<TKey, TValue> _options;
        private readonly ConcurrentDictionary<TKey, CacheEntry<TKey, TValue>> _entries;
        ...
        public MemoryCache(IOptions<MemoryCacheOptions<TKey, TValue>> optionsAccessor)
        ...
        public MemoryCache(IOptions<MemoryCacheOptions<TKey, TValue>> optionsAccessor, ILoggerFactory loggerFactory)
        {
            ...
            _entries = new ConcurrentDictionary<TKey, CacheEntry<TKey, TValue>>();
        ...
        private ICollection<KeyValuePair<TKey, CacheEntry<TKey, TValue>>> EntriesCollection => _entries;

        /// <inheritdoc />
        public ICacheEntry<TKey, TValue> CreateEntry(TKey key)
        {
            CheckDisposed();
            ValidateCacheKey(key);

            return new CacheEntry<TKey, TValue>(key, this);
        }

        internal void SetEntry(CacheEntry<TKey, TValue> entry)
        {
            if (_disposed)
            {
            ...
            if (_entries.TryGetValue(entry.Key, out CacheEntry<TKey, TValue>? priorEntry))
            {
        ...
        public void Remove(TKey key)
        {
            ...
            CheckDisposed();
            if (_entries.TryRemove(key, out CacheEntry<TKey, TValue>? entry))
            {
                if (_options.SizeLimit.HasValue)
                {
                    Interlocked.Add(ref _cacheSize, -entry.Size!.Value);
        ...
        private void RemoveEntry(CacheEntry<TKey, TValue> entry)
        {
            if (EntriesCollection.Remove(new KeyValuePair<TKey, CacheEntry<TKey, TValue>>(entry.Key, entry)))
            {
                if (_options.SizeLimit.HasValue)
                {
                    Interlocked.Add(ref _cacheSize, -entry.Size!.Value);
        ...
        internal void EntryExpired(CacheEntry<TKey, TValue> entry)
        {
        ...
        private void StartScanForExpiredItemsIfNeeded(DateTimeOffset utcNow)
        {
            ...
            void ScheduleTask(DateTimeOffset utcNow)
            {
                ...
                Task.Factory.StartNew(state => ScanForExpiredItems((MemoryCache<TKey, TValue>)state!), this,
        ...
        private static void ScanForExpiredItems(MemoryCache<TKey, TValue> cache)
        {
            ...
            foreach (CacheEntry<TKey, TValue> entry in cache._entries.Values)
            {
        ...
        private bool UpdateCacheSizeExceedsCapacity(CacheEntry<TKey, TValue> entry)
        {
            if (!_options.SizeLimit.HasValue)
            {
                return false;
        ...
        private void TriggerOvercapacityCompaction()
        {
            ...
            ThreadPool.QueueUserWorkItem(s => OvercapacityCompaction((MemoryCache<TKey, TValue>)s!), this);
        }
        ...
        private void Compact(long removalSizeTarget, Func<CacheEntry<TKey, TValue>, long> computeEntrySize)
        {
            var entriesToRemove = new List<CacheEntry<TKey, TValue>>();
            var lowPriEntries = new List<CacheEntry<TKey, TValue>>();
            var normalPriEntries = new List<CacheEntry<TKey, TValue>>();
            var highPriEntries = new List<CacheEntry<TKey, TValue>>();
            long removedSize = 0;
            ...
            foreach (CacheEntry<TKey, TValue> entry in entriesToRemove)
            {
                RemoveEntry(entry);
            }
            ...
            // ?. Items with the soonest sliding expiration.
            // ?. Larger objects - estimated by object graph size, inaccurate.
            static void ExpirePriorityBucket(ref long removedSize, long removalSizeTarget, Func<CacheEntry<TKey, TValue>, long> computeEntrySize, List<CacheEntry<TKey, TValue>> entriesToRemove, List<CacheEntry<TKey, TValue>> priorityEntries)
            {
                ...
                foreach (CacheEntry<TKey, TValue> entry in priorityEntries)
                {
                    entry.SetExpired(EvictionReason.Capacity);
                    entriesToRemove.Add(entry);
                    removedSize += computeEntrySize(entry);
