...
{
    internal partial class CacheEntry<TKey, TValue> : ICacheEntry<TKey, TValue> where TKey : notnull
    {
        ...
        private long? _size;
        private CacheEntry<TKey, TValue>? _previous; // this field is not null only before the entry is added to the cache
        private TValue? _value;
        private CacheEntryState _state;

        internal CacheEntry(TKey key, MemoryCache<TKey, TValue> memoryCache)
        {
            Key = key ?? throw new ArgumentNullException(nameof(key));
            _cache = memoryCache ?? throw new ArgumentNullException(nameof(memoryCache));
            _previous = CacheEntryHelper<TKey, TValue>.EnterScope(this);
        ...
        /// </summary>
        public IList<PostEvictionCallbackRegistration<TKey, TValue>> PostEvictionCallbacks => GetOrCreateTokens().PostEvictionCallbacks;
        ...
        public bool Expired => _state.IsExpired;
        ...
        public void Dispose()
        {
            if (!_state.IsDisposed)
            {
                ...
                CacheEntryHelper<TKey, TValue>.ExitScope(this, _previous!);
        ...
        private static void ExpirationTokensExpired(object obj)
        {
            ...
            {
                var entry = (CacheEntry<TKey, TValue>)state!;
        ...
        internal void PropagateOptions(CacheEntry<TKey, TValue> parent)
        {
            if (parent == null)
            {
                return;
